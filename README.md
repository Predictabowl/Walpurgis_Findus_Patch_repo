...
* **Primary Attack (Base & Upgraded)**
  * Drains 0.7 Green mana every swing, but only if it hit an enemy. If the player does not have enough mana available, the attack suffers approximately -20% damage and range penalties, and lifesteal on hit is disabled.
...
* **Primary Attack (Triple Projectiles - Mutation)**
  Consumes 1G mana per swing, plus 0.2G mana if it hits (for a total of 1.2 mana per swing). Projectiles will no longer spawn if mana is depleted.
  > **RPG Lore:** The Bardiche uses a small amount of magical energy to manifest ethereal projectiles during the swing.
  >
  > ---
  >
  > **Considerations:** While it has less AoE potential compared to the upgraded version, if you use the 3 projectiles as a shotgun it's a massive single burst damage increase, that reduces by a LOT the TTK (Time to Kill). Plus you have ranged projectiles to finish off weakened enemies from a distance. If you pair with the corpse explosion tertiary, it will cost 2.2 mana per swing, but with shorter TTK and this earlier corpse availability, I believe it's worth it.
